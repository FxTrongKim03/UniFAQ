...
class ChatContainer {
    ...
    async callGeminiAPI(userMessage) { // <--- userMessage được dùng ở đây
        ...
        // --- SỬA: Bao gồm userMessage trong prompt ---
        const prompt = `Bạn là trợ lý ảo cho một trường đại học tên là UniFAQ.
                       ...
                       Câu hỏi của người dùng: "${userMessage}"`; // <-- Sử dụng userMessage

        const requestBody = { // <-- requestBody được dùng ở dưới
        ...
        try {
            const response = await fetch(API_URL, {
                ...
                // --- SỬA: Thêm requestBody vào fetch body ---
                body: JSON.stringify(requestBody), // <-- Sử dụng requestBody
            });
